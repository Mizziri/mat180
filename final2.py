...
def dp_kmeans(data, k, Delta, epsilon, delta, s, patience, initial_centroids):
    centroids = initial_centroids
    ...
    while num_iterations_diverging < patience:
        ...
        for point in data:
            ...
            distances += rng.normal(0, 2 * k * math.log(1.25/(delta * (s-1))) * ((Delta / (epsilon * (s-1))) ** 2), k)
...
def main():
    ...
    for _ in range(300):
        ...
        for eps in epsilons:
            ...
            pred_centroids = dp_kmeans(data, 4, math.sqrt(2), eps, 10 ** -64, 2, 3, initial_centroids[:])
